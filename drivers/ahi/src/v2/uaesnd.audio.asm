...
debug_setvol
	...
	; the hardware plays each (mono) set on both speakers at the
	; full set volume, while the AHI mixer gives a center-panned
	; channel volume*0.5 per speaker, so halve the volume here
	; (>>16 instead of >>15) to match AHI loudness

scale_output_volume
	moveq #0,d0
	move.w d1,d0
	mulu.w p_OutputVolume(a2),d0
	lsr.l #8,d0
	lsr.l #8,d0
	rts
...
update_channel_volumes
	...
	move.l p_Base(a2),a0
	add.w #STREAM_START,a0
	move.l p_AudioCtrl(a2),a3
	move.w ahiac_Channels(a3),d3
	subq.w #1,d3
	bmi.s .done
.loop
	moveq #0,d1
	move.w ch_volume(a1),d1
	bsr.s scale_output_volume
	move.l ch_set_current(a1),a3
	move.w d0,set_volume(a3)
	move.w d0,UAESNDSetCurrent+set_volume(a0)
...
.update_channel
	...
	; interrupt on set start and on repeat start, so the AHI
	; SoundFunc also runs when a sound loops without a queued
	; successor (ahi.device relies on this to chain buffers)

	move.b #%101,set_intena(a4)

	; mono streams: route to both left and right output channels
	; (the hardware cannot exclude the left channel, so this is
	; the only routing that keeps stereo output balanced)

	move.b #3,set_mask(a4)
	move.l a4,set_next_pointer(a4)
	or.b #%101,stream_master_intena(a5)
...
.doimm
	...
	; start stream immediately, or defer grouped starts while AHI has the driver disabled
	moveq #1,d0
	move.l d5,d1
	lsr.l #8,d1
	lsl.l d1,d0
	move.l p_Base(a3),a0
	or.l d0,base_stream_enable(a0)
	tst.w p_DisableCount(a3)
	beq.s .start_now
	or.l d0,p_PendingStartMask(a3)
	bra.s .noimm
.start_now
	or.l d0,base_stream_intena(a0)
...
flush_pending_starts
	...
	move.l p_PendingStartMask(a1),d2
	beq.s .done
	clr.l p_PendingStartMask(a1)
	move.l p_Base(a1),a0
	or.l d2,base_stream_enable(a0)
	add.w #STREAM_START,a0
	move.l p_Channels(a1),a2
	moveq #0,d1
.loop
	btst d1,d2
	beq.s .next
	move.l ch_set_current(a2),stream_sample_pointer_imm(a0)
.next
	add.w #STREAM_OFFSET,a0
	add.w #channel_SIZEOF,a2
	addq.w #1,d1
	cmp.w p_StreamCnt(a1),d1
	bne.s .loop
.done
...
AHIsub_AllocAudio
	...
	beq .error
	move.l d0,a3
	move.l #UAESND_SIZEOF,p_DriverDataSize(a3)
...
get_tag_audioid
	...
	beq.s .no_tags
	move.l a6,-(sp)
	move.l ub_UtilBase(a6),a6
	move.l #AHIA_AudioID,d0
	moveq #0,d1
	move.l a1,a0
	call GetTagData
	tst.l d0
	bne.s .got_audioid
	move.l #AHIDB_AudioID,d0
	moveq #0,d1
	move.l a1,a0
	call GetTagData
.got_audioid
...
get_audioid_bits
	bsr.w get_tag_audioid
	cmp.l #$003b0001,d0
	bne.s .not_stereo_bits
	moveq #16,d0
	rts
.not_stereo_bits
	moveq #-1,d0
...
tag_frequencies
	...
	dc.l 6000	; DAT/8
	dc.l 7350	; CD/6
	dc.l 8000	; u-Law and A-Law, DAT/6
	dc.l 9600	; DAT/5
	dc.l 11025	; CD/4
	dc.l 12000	; DAT/4
	dc.l 14700	; CD/3
...
.notimer
	...
	; sample started or repeat started interrupt?
	and.b #%101,d2
	beq.s .next
